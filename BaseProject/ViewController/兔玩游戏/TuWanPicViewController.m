...
- (void)viewDidLoad {
    ...
    [self.tuwanPicVM getDataFromNetCompleteHandle:^(NSError *error) {
        [self hideProgress];
        //创建图片展示页面，Github上面排名最靠前的图片展示类控件
        MWPhotoBrowser *photoB = [[MWPhotoBrowser alloc]initWithDelegate:self];
        //图片展示页面不应该是当前页面推出，而是取代当前页面在导航控制器中的位置
        NSMutableArray *naviCs = [NSMutableArray arrayWithArray:self.navigationController.viewControllers];
        //删除最后一个控制器，即当前的白色控制器
        [naviCs removeLastObject];
        //把图片控制器添加到最后
        [naviCs addObject:photoB];
        //吧新的控制器数组赋值给导航控制器
        self.navigationController.viewControllers = naviCs;
    }];
}
